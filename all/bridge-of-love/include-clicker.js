...
WScript=function(script)
{
	...
	if(storage)
	{
		...
		if(typeof storage.black=="undefined")
			storage={black:{},writers : {},goal:"textual",af:30,at:50,text:"",ids:[],template:[],times:0,photos:[]};
		else
		{
			if(storage.goal)
		...
		Curiosity = function ()
		{
			...
			if(queue2.length>0)
			{
				$.each(queue2.slice().reverse(),function(i,v){
					if(!(v.next in storage.template) || $("#id_contact_item_"+v.id).length==0)
					...
					else if(v.time<time)
					{
						...
						age=v.age;
						name=v.name;
						text=storage.template[v.next++];

						v.time=time+(t==0 ? 20000+(Math.random()*100000) : t*60000);

						$("#id_contact_item_"+v.id).click();
			...
			if(!man)
			{
				...
				//Рассылка по онлайну
				if(storage.goal == "textual" || storage.goal == "template")
				{
					$("#profilecontainer > li").each(function(){
						name=$(".name",this).text().trim();
						age=($(".age",this).text().match(/\d+/)||[0]).pop()-0;
						id=$(this).attr("id")-0;

						if((age==0 || storage.af<=age && age<=storage.at) && storage.ids.indexOf(id)==-1 && !(id in storage.black))
						{
							man=true;
							$(this).click();
							storage.ids.push(id);
				...
				//Рассылка по писателям
				else if(writers_ids.length>0)
				{
					id=writers_ids.pop();

					var	span=$("#profilepages span"),
						items=$("#profilecontainer > li[id="+id+"]").each(function(){
							name=$(".name",this).text().trim();
							age=($(".age",this).text().match(/\d+/)||[0]).pop()-0;
							id=$(this).attr("id")-0;
					...
					if(!man)
					{
						//Мужик не найден на первой странице
						if(items.length==0 && span.next().is("i"))
						{
							span.next().click();
							tos=setTimeout(Curiosity,1000);

							return;
						}

						//Мужик найден, но не подошел или ащще не найден
						if(writers.length>0)
						{
							tos=setTimeout(Curiosity,500);
							return;
						}
					}
			...
			$("#message").val(
				text.replace(/{hi\|hello\|hey}/ig, GetHiVar(hi_var++) )
				.replace(/{name}/ig,name).replace(/{name1}/ig, Name1(name)).replace(/{name2}/ig, Name2(name))
				.replace(/{age}/ig,age)
			);
			setTimeout(function(){
				if( $("#user-id").text().indexOf(id.toString())>-1 )
				{
					$(".b-button-send").click();
					Status(storage.ids.length);
				}
			...
			if(send_template && storage.template.length>1 && add)
			{
				var t=times.val();
				...
				queue2.push({
					...
					name:name,
					next:1,
					time:time + (t==0 ? 20000+(Math.random()*100000) : t*60000),
				});
			}

			//$.get("//ukrainiangirls.pw/get.php", {name: name, stat:"sent"});

			tos=setTimeout(Curiosity,3000 + 1200*storage.photos.length );
        };
    ...
    run.click(function(e){
        ...
        else
        {
			...
			writers_ids=Object.keys(storage.writers);
